...
const u8_t modbusCommon_CRC16Hi[] =
{
...
const u8_t modbusCommon_CRC16Lo[] =
{
...
{
	u16_t i = 0;
	memset(adu->bytes, 0, sizeof(adu->bytes));
...
u16_t ModbusCommon_CalculateCRC16(ModbusAdu* data)
{
	...
	for(i = 0; i < data->length - 2; i++)
	{
		...
		CRC16Lo = CRC16Hi ^ modbusCommon_CRC16Hi[index];
		CRC16Hi = modbusCommon_CRC16Lo[index];
	}
...
u8_t ModbusCommon_CheckCRC16(ModbusAdu* data)
{
	if(data->length > 2)
	{
		...
		u16_t calculateCRC = ModbusCommon_CalculateCRC16(data);
		if(crc == calculateCRC)
